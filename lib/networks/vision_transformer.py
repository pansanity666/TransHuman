...
class RandomMaskingGenerator:
    def __init__(self, input_size=6890, mask_ratio=(0.1, 0.5), mask_prob=0.5):
        
        assert type(mask_ratio) == tuple
...
# Non-learnable position encoding from pixelNeRF
class PositionalEncoding(torch.nn.Module):
    ...
    def __init__(self, num_freqs=6, d_in=3, freq_factor=np.pi, include_input=True):
        ...
        self.d_out = self.num_freqs * 2 * d_in # 3* 6 * 2 + 3 = 39
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, use_learnable_pos_emb=True, num_classes=0, embed_dim=768, depth=12,
        ...
        self.PE = PositionalEncoding(num_freqs=int(embed_dim//6), include_input=False)
    ...
    def get_PE(self, PE, device, out_shape):
        
        B, N, C = out_shape
        pe = self.PE(PE.flatten(0,1)).view(B, N, -1)

        return pe

    def prepare_tokens(self, x, PE, mask):
        # x: B, N, C
        # PE: B, N, 3
        B, N, C = x.shape
        if not mask is None and not mask.sum()==0:   # mask.sum()==0 when maskraito = 0, 
            x[mask] = self.mask_token    
        pe = self.get_PE(PE, x.device, x.shape)
        x = x + pe
    ...
    def forward(self, x, PE, mask=None):
        ...
        x = self.prepare_tokens(x, PE, mask) 
...
def vit_tiny( depth, **kwargs):
    model = VisionTransformer(embed_dim=192, depth=depth, num_heads=3, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def vit_small( depth, **kwargs):
    model = VisionTransformer( embed_dim=384, depth=depth, num_heads=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def vit_base( depth, **kwargs):
    model = VisionTransformer(embed_dim=768, depth=depth, num_heads=12, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
