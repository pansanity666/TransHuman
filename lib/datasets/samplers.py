...
class FrameSampler(Sampler):
    ...
    def __init__(self, dataset):
        ...
        human_info = dataset.human_info      
        self.inds = []
        inds = np.arange(0, len(dataset.ims))

        start = 0
        for human in human_info.keys():
        
            num_frames, num_cams = dataset.human2Nframe_Ncam[human]
            sub_len = num_frames * num_cams
            sub_inds = inds[start:start + sub_len]

            if cfg.test.full_eval: 
                sub_inds = sub_inds.reshape(num_frames, -1)[::1] # test target frames is determined here. 
            else:
                sub_inds = sub_inds.reshape(num_frames, -1)[::30] # test target frames is determined here.
