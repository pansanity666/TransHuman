...
class Dataset(data.Dataset):
    def __init__(self, data_root, split):
        ...
        human_info = get_human_info.get_human_info_nhr()
        ...
        for idx in range(len(human_list)):
            ...
            ann_file = os.path.join(self.data_root, human, 'annots.npy') ### Change
            ...
            cam_names = [n.split('/')[1] for n in annots['ims'][0]['ims']]
            # print(cam_names) ['0000', '0001', '0002', '0003', '0004', '0005', '0006', '0007', '0008', '0009', '0010', '0011', '0012', '0013', '0014', '0015', '0016', '0017', '0018', '0019', '0020', '0021', '0022', '0023', '0024', '0025', '0026', '0027', '0028', '0029', '0030', '0031', '0032', '0033', '0034', '0035', '0036', '0037', '0038', '0039', '0040', '0041', '0042', '0043', '0044', '0045', '0046', '0047', '0048', '0049', '0050', '0051', '0052', '0053', '0054', '0055']
        ...
        ### LBS and JOints not supported yet. 
    ...
    def get_mask(self, human, camera, frame):
        ...
        # msk_path = os.path.join(self.data_root, human, 'mask', camera, frame)[:-4] + '.jpg'
        # # print(msk_path) ./data/NHR/sport_1_easymocap/schp/0018/000000.png
        # msk_exist = os.path.exists(msk_path)
        # if msk_exist:
        #     msk = imageio.imread(msk_path)
        #     msk = (msk != 0).astype(np.uint8)

        # msk_path = os.path.join(self.data_root, human, 'schp', camera, frame)[:-4] + '.png'
        # # print(msk_path) ./data/NHR/sport_1_easymocap/schp/0018/000000.png
        ...
        # print(msk.shape) #  (1024, 1224)
        # print(msk.min(), msk.max()) #  0, 100
    ...
    def prepare_input(self, human, i):
        ...
        if 'basketball' in human:
            vertices_path = os.path.join(self.data_root, human, 'vertices', '{}.npy'.format(str(i).zfill(6)))
        else:
            vertices_path = os.path.join(self.data_root, human, 'vertices', '{}.npy'.format(i))
        xyz = np.load(vertices_path).astype(np.float32)
        
        ### DEBUG
        ### apply transform to smpl world coordinate
        transform = self.human_info[human]['transform']
        xyz = (np.dot(xyz, transform[:3, :3].T) + transform[:3, 3]).astype(np.float32)
        ...
        # print(smpl_vertices.shape)
        # write_point_cloud('./smplworld_bfr_transform.ply', smpl_vertices, )
        # transform = self.human_info[human]['transform']
        # smpl_vertices = (np.dot(smpl_vertices, transform[:3, :3].T) + transform[:3, 3]).astype(np.float32)
        # print(smpl_vertices.shape)
        # write_point_cloud('./smplworld_aft_transform.ply', smpl_vertices, )
        # assert False 
        ...
        params_path = os.path.join(self.data_root, human, 'output-smpl-3d', 'smplfull', '{}.json'.format(str(i).zfill(6)))
        
        ### load json-like
        with open(params_path) as json_file:
            params = json.load(json_file)['annots'][0]

        # print(xyz.shape) # 6890, 3 
        Rh = np.array(params['Rh']) # 1, 3  
        Th = np.array(params['Th']).astype(np.float32) # The center of SMPL under the world coordinate. 
        
        # convert rotation vector to rotation matrix
        R = cv2.Rodrigues(Rh)[0].astype(np.float32) # 3, 3 # The vect of SMPL coordinate. [r1 | r2 | r3]
        # Th = params['Th']
        xyz = np.dot(xyz - Th, R)
        # print(Rh.shape) # 1, 3 
        # print(Th.shape) # 1, 3 
        ...
        ##### TODO
        # patch_size = 0.05
        # Trans_patch_max = np.ceil((dhw - min_dhw) / patch_size).astype(np.int32) # (6890, 3), voxelized coordinate for SMPL vertex. 
        # Trans_patch_min = np.floor((dhw - min_dhw) / patch_size).astype(np.int32)

        # plotPoints(Trans_patch_max,'./test_{}.jpg'.format(patch_size))
        # print(Trans_patch_max)
        # print(Trans_patch_min)
        # assert False
        ...
        params['poses'] = np.array(params['poses'])
        params['shapes'] = np.array(params['shapes'])
    ...
    # DEBUG
    def tranform_RT(self, R, T, transform):
    ...
    def load_img_msk(self, human, camera, frame, cam_ind):
        # load and resize 
        img_path = os.path.join(self.data_root, human, 'images', camera, frame)
        ...
        # print(R.shape, T.shape) # (3,3); (3,1)
        
        ### transform R, T
        R, T = self.tranform_RT(R, T, self.human_info[human]['transform'])
    ...
    def __getitem__(self, index):
        ...
        # print(img_path)
        human = img_path[-4] # change 
        camera = img_path[-2]
        frame = img_path[-1]
        cam_ind = self.cam_inds[index]
        # print(human, camera, frame) # 
        ...
        for t in range(cfg.time_steps):
            ...
            for i in range(num_inputs):
                ...
                input_img_path = os.path.join(self.data_root, human, 'images', cam_name, filename)
                tmp_input_img_pths.append(input_img_path)
                
                # print(human, cam_name, filename) # sport_1_easymocap 0001 000000.jpg
                ...
                if cfg.depth_map:
                    depthmap_pth = os.path.join(cfg.depth_root, human, 'images', cam_name,
                ...
                ### apply transform to RT
                # print(in_R.dtype) float32
                in_R, in_T = self.tranform_RT(in_R, in_T, self.human_info[human]['transform'])
