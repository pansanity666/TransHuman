...
class Dataset(data.Dataset):
    def __init__(self, data_root, split):
        ...
        for idx in range(len(human_list)):
            ...
            ### 
            if self.split == 'test':
                test_view = cfg.test_target_view
            else:
    ...
    def prepare_input(self, human, i):
        ...
        vertices_path = os.path.join(self.data_root, human,  'new_vertices', '{}.npy'.format(i))
        ...
        params_path = os.path.join(self.data_root, human,  'new_params', '{}.npy'.format(i))
        params = np.load(params_path, allow_pickle=True).item()
        # print(params.keys()) # dict_keys(['poses', 'Rh', 'Th', 'shapes'])

        # print(xyz.shape) # 6890, 3 
        R = params['R'].astype(np.float32)  # 1, 3  s
        ...
        _, _, blended_mtx = self.smpl(params['poses'].T, params['shapes'])
        ...
        return feature, coord, out_sh, can_bounds, bounds, R, Th, center, rot, trans, smpl_vertices, smpl_vertice_smplcoord, min_xyz_smpl, max_xyz_smpl, blended_mtx, params['poses'], params['shapes']
    ...
    def load_img_msk(self, human, camera, frame, cam_ind):
        # load and resize 
        img_path = os.path.join(self.data_root, human, camera, frame)
        ...
        T = np.array(self.cams[human]['T'][cam_ind])  ### NOTE: Unlike ZJU-MoCap THU no need / 1000.
    ...
    def __getitem__(self, index):
        ...
        for t in range(cfg.time_steps):
            ...
            for i in range(num_inputs):
                ...
                if cfg.depth_map:
                    depthmap_pth = os.path.join(cfg.depth_root, human, cam_name,
                ...
                in_T = (np.array(self.cams[human]['T'][idx]) ).astype(
                    np.float32)
        ...
        R, Th, center, rot, trans, tmp_smpl_vertices, smpl_vertice_smplcoord, \
